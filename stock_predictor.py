...
st.markdown("<h1 style='text-align: center; color: #2c3e50;'>Stock Price Prediction for Nifty 50 Stocks (June 13 - July 12, 2025)</h1>", unsafe_allow_html=True)

# Sidebar for user input with styling
st.sidebar.markdown("<h3 style='color: #2c3e50;'>Select Options</h3>", unsafe_allow_html=True)
selected_stock = st.sidebar.selectbox("Choose a Stock", stocks, format_func=lambda x: f"**{x}**")
selected_date = st.sidebar.selectbox("Choose a Date", future_dates, format_func=lambda x: f"**{x}**")
...
st.markdown(f"<h2 style='color: #27ae60;'>Predicted Price for {selected_stock} on {selected_date}</h2>", unsafe_allow_html=True)
st.markdown(f"<p style='font-size: 24px; color: #e74c3c; font-weight: bold;'>Predicted Price: {predicted_price:.2f} INR</p>", unsafe_allow_html=True)

# Optional: Toggle for full table with styling
if st.checkbox("Show Full Prediction Table", key="table_toggle"):
    prediction_df = pd.DataFrame(index=stocks, columns=future_dates)
    for stock in stocks:
        prediction_df.loc[stock] = predictions[stock]
    st.markdown("<h3 style='color: #2980b9;'>Full Prediction Table (June 13 - July 12, 2025)</h3>", unsafe_allow_html=True)
    st.table(prediction_df.style.set_properties(**{'text-align': 'center', 'border': '1px solid #ddd', 'padding': '8px'}).background_gradient(cmap='Blues').format("{:.2f}"))

# Add a summary with styling
st.markdown("<h3 style='color: #8e44ad;'>Summary of Predictions</h3>", unsafe_allow_html=True)
for stock in stocks:
    st.write(f"**{stock}:** {predictions[stock][:5]}... (22 total)")

# Final Report with styling
st.markdown("<h3 style='color: #c0392b;'>Final Report</h3>", unsafe_allow_html=True)
st.markdown("""
